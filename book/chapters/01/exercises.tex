...
is one program: create a directory for it with the Makefile from \secref{c1:sec:makefile}, and build
and run it with \code{make}. Check your output against the example output each exercise gives.

The solutions are in \secref{sol:sec:c1}, one program per set. Try each exercise yourself before
looking at them.
\end{aside}
