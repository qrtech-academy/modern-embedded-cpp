...
\secref{c5:sec:class}. Sets~1 and~3 are one program each: write each in its own directory under
\file{lectures/L05/exercises} in the companion repository, with the file names its README gives, and
build and run it with the Makefile from \secref{c1:sec:makefile} (or the one from
\secref{c1:sec:tips}, for a program with an \file{include} directory).

\textbf{How to check your work.} Run \code{make test} from the repository root. A set's tests switch
on as soon as its files exist, and compare your program's output with the expected output each
exercise gives, character for character; a set you have not started is reported as \code{SKIP}.

The solutions are published in the companion repository, under
\file{lectures/L05/appendix/solutions}: \file{function_template} for Set~1 and \file{class_template}
for Set~3, with the answers to the reflection questions in the README beside them. Set~2 has no
program; its answers are the subject of \secref{c5:sec:staticassert} and
\secref{c5:sec:whycompiletime}. Try each exercise yourself before looking at
them.
\end{aside}
