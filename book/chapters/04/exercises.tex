...
same program into one with a smart-pointer factory. Write the first in
\file{lectures/L04/exercises/raw_factory} and the second in
\file{lectures/L04/exercises/smart_factory}, in the companion repository, each with the layout
below, and build and run them with the Makefile from \secref{c1:sec:tips}, which already supports
the \file{include} and \file{source} directories.

\textbf{How to check your work.} Run \code{make test} from the repository root. A set's tests
switch on as soon as its files exist, and check the names, signatures and deleted operations
exactly; they compare the ESP32-S3 driver's output with the example in
Exercise~\ref{c4:ex:1.3} character for character, and count what your logic class creates and
deletes. A set you have not started is reported as \code{SKIP}.

The solutions are published in the companion repository, under
\file{lectures/L04/appendix/solutions}: \file{raw_factory} for Sets~1 to~3 and \file{smart_factory}
for Set~4, with the answers to the reflection questions in the README beside them. Try each exercise
