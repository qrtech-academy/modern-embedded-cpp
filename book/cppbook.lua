-- cppbook.lua - the one piece of the design that TeX macros cannot do cleanly.
--
...
-- "\n ". Both are undone here, and a line break is allowed after a scope operator or a comma, so
-- a name like std::is_integral<T>::value can break at a :: rather than run into the margin.

local catcode_other = -2

function cppbook_code(s)
  s = s:gsub("#+", "#")
  s = s:gsub("(\\%a+) ", "%1")
