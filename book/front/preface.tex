...
\begin{booktable}{@{}p{7.4em}L@{}}
  ...
  \midrule
  \kindtag{Code} & Write C++, compile it, and check it with \code{make test}, which also compares
    its output with the one the exercise gives. \\
...
Makefile every program in the book is built with. Then clone the companion repository, together
with the test framework it keeps as a submodule:
...
make test                # Test your exercise solutions.
...
where each lecture's README says which file goes where. \code{make test} then runs every
lecture's tests against them: a set you have not started is reported as \code{SKIP}, with the file
its tests are waiting for, and a set you have written is tested, including its output, which is
compared with the example output in the exercise character for character. \code{make test
SOLUTIONS=1} runs the same tests against the reference solutions.
