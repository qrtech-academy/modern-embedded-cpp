...
to those exercises and the two written papers described below.
...
one, and each exercise in a set adds a piece of it; the solutions in
Appendix~\ref{sol:ch:solutions} follow the same grouping. Every exercise is labelled with its
kind:
...
\textbf{Try each exercise before you read its solution.} The solutions are the reference
implementations from the course repository, typeset straight from their source files so that the
book and the code can never disagree. Where a solution has a design decision worth discussing, or
a reflection question to answer, the discussion is printed beside the code.
