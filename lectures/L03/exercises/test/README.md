...
That makes `sendMessage()` callable from the test, and your program runnable as
`exercise4Main()`. Renaming `main` has one side effect: `main` is the only function allowed to leave
out its `return`, so the test silences that one warning for your file.
